...
const NotificationDropdownMenu = forwardRef<HTMLDivElement, NotificationDropdownMenuProps>(({ notifications }, ref) => {
    ...
    const handleMarkAsRead = async (notificationId: string) => {
        ...
        if (notification && notification.entity_type === NotificationEntity.job) {
            const jobId = notification.entity_id?.trim();
            const openApplications =
                notification.type === NotificationType.bidReceived ? "&openApplications=1" : "";
            if (jobId) {
                router.push(`/dashboard?view=my-jobs&jobId=${jobId}${openApplications}`);
            } else {
        ...
        if (notification && notification.entity_type === NotificationEntity.bid) {
            if(userInfo.role === "freelancer") {
                ...
                const jobId = (notification.payload as { job_id?: string } | null)?.job_id;
                if (jobId) {
                    router.push(`/dashboard?view=my-jobs&jobId=${jobId}&openApplications=1`);
                } else {
                    router.push(`/dashboard?view=my-jobs&openApplications=1`);
                }
            }
        }  
        ...
        const isClientBidNotification =
            userInfo.role !== "freelancer" &&
            ((notification?.entity_type === NotificationEntity.bid) ||
                (notification?.entity_type === NotificationEntity.job &&
                    notification?.type === NotificationType.bidReceived));
