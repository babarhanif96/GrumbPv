...
interface userJobOrGigPostProps {
    ...
    /** Auto-open the Applications modal (e.g. from bid notification deep link). */
    autoOpenApplications?: boolean;
}
...
const UserJobOrGigPost = ({ job_id, gig_id, description, title, location, tags, image, minBudget, maxBudget, currency, deadline, status, link, variant = "job", hasBids = false, bidsCount = 0, hasUnreadBidNotification = false, onApplicationsOpen, autoOpenApplications = false }: userJobOrGigPostProps) => {
    ...
    useEffect(() => {
        if (autoOpenApplications && variant === "job" && job_id && !hasAutoOpenedRef.current) {
            hasAutoOpenedRef.current = true;
            void handleApplications();
        }
    }, [autoOpenApplications, variant, job_id]);
