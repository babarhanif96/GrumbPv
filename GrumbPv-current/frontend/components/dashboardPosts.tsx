...
import { approveWork, buyerJoinDispute, confirmMilestoneRewards, createChainTx, deliverWork, fundEscrow, getExpectedMilestoneRewards, getJobApplicationById, increaseFund, increaseWithdraw, initiateDispute, updateJobMilestone, updateJobStatusById, updateUserFunds, venderPayDisputeFee, withdrawFunds } from "@/utils/functions";
import { ensureSignerMatchesSession } from "@/utils/walletAuth";
...
const DashboardPosts = ({ user, jobMilestoneId, title, description, milestoneStatus, ipfs, variant, applicationDocId, clickHandler }: DashboardPostsProps) => {
    ...
    const requireMatchingWallet = useCallback(async () => {
        const auth = await ensureSignerMatchesSession(address, userInfo, setUserInfo);
        if (!auth.ok) {
    ...
    const handleFund = async () => {
        try {
            if (!(await requireMatchingWallet())) return;
            const result = await fundEscrow(user.id, jobMilestoneId, Number(CONFIG.chainId));
            if (!result.success || !result.data) {
                toast.error(result.error || "Failed to prepare funding transaction", {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
            ...
            if (result.data.approvalTx) {
                const { hash: approvalHash, error: approvalError } = await sendTransaction({
                    to: result.data.approvalTx.to,
                    data: result.data.approvalTx.data,
                    value: result.data.approvalTx.value,
                    chainId: Number(result.data.approvalTx.chainId),
                });
                if (!approvalHash) {
                    toast.error(approvalError || "Failed to approve token spending", {
                        position: "top-right",
                        autoClose: 5000,
                        hideProgressBar: false,
                        closeOnClick: true,
    ...
    const handleWithdraw = async () => {
        ...
        if (updatedJobMilestone.success) {
            ...
            const rewardConfirm = await confirmMilestoneRewards(jobMilestoneId, txHash);
            if (rewardConfirm.success && rewardConfirm.data?.distributed) {
                const perSide = rewardConfirm.data.buyerReward
                    ? (Number(rewardConfirm.data.buyerReward) / 1e18).toFixed(4)
                    : null;
                toast.success(
                    perSide
                        ? `Funds withdrawn. GRMPS rewards sent to client and freelancer (${perSide} each).`
                        : "Funds withdrawn. GRMPS rewards distributed to both parties.",
                    {
                        position: "top-right",
                        autoClose: 7000,
                        hideProgressBar: false,
            ...
            } else if (rewardConfirm.success && rewardConfirm.data?.skipped) {
                toast.warn("Funds withdrawn, but GRMPS rewards were skipped. Contact support if this was unexpected.", {
                    position: "top-right",
                    autoClose: 7000,
                    hideProgressBar: false,
                    closeOnClick: true,
