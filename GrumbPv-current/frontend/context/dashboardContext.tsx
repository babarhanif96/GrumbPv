...
import { getBidById, getBidsByJobId, getConversationById, getDashboardDataByUserId, getGigById, getJobApplicationById, getJobBidForClientById, getJobById, getJobMilestoneByEscrowAddress, getJobMilestoneById, getUserById, updateNotification } from "@/utils/functions";
...
export const DashboardProvider = ({ children }: Props) => {
    ...
    const jobIdsWithUnreadBidNotification = useMemo(() => {
        ...
        for (const n of notificationsInfo) {
            ...
            if (n.entity_type === NotificationEntity.job && n.type === NotificationType.bidReceived) {
                set.add(n.entity_id);
                continue;
            }
            if (n.entity_type !== NotificationEntity.bid) continue;
    ...
    const markBidNotificationsAsReadForJob = async (jobId: string) => {
        const toMark = notificationsInfo.filter(n => {
            ...
            if (n.entity_type === NotificationEntity.job && n.type === NotificationType.bidReceived && n.entity_id === jobId) {
                return true;
            }
            if (n.entity_type !== NotificationEntity.bid) return false;
    ...
    useEffect(() => {
        ...
        const handleNotification = async (notification: DashboardNotification) => {
            ...
            if (
                ...
                if (notification.type === NotificationType.bidReceived && userInfo.role !== "freelancer") {
                    const bidsRes = await getBidsByJobId(notification.entity_id);
                    if (!bidsRes.success || !bidsRes.data) return;

                    setJobsInfo(prev =>
                        prev.map(job => {
                            if (job.id !== notification.entity_id) return job;
                            return { ...job, bids: bidsRes.data };
                        })
                    );
                    return;
                }
