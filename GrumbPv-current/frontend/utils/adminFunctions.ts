...
import {
  ...
  AdminSystemSettings,
  Pagination,
  AdminDisputeMilestone,
  AdminDisputesSummary,
} from '@/types/admin';
...
export const getAdminDisputes = async (params?: {
  ...
  try {
    const response = await adminApi.get('/admin/disputes', { params });
    return {
      success: true,
      data: response.data.data as AdminDisputeMilestone[],
      pagination: response.data.pagination as Pagination,
      summary: response.data.summary as AdminDisputesSummary,
    };
  } catch (error: any) {
    return {
      success: false,
      error: error.response?.data?.error?.message || error.message || 'Failed to get disputes',
    };
  }
};
