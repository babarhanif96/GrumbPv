...
export const getExpectedMilestoneRewards = async (job_milestone_id: string) => {
    try {
        const response = await EscrowBackend.get(`/contract/escrow/${job_milestone_id}/rewards/expected`);
        return { success: true, data: response.data.data };
    } catch (error: any) {
        return {
            success: false,
            error: error.response?.data?.error?.message || error.message || 'Failed to get expected rewards',
        };
    }
};

export const confirmMilestoneRewards = async (job_milestone_id: string, txHash: string) => {
    try {
        const response = await EscrowBackend.post(`/contract/escrow/${job_milestone_id}/rewards/confirm`, { txHash });
        return { success: true, data: response.data.data };
    } catch (error: any) {
        return {
            success: false,
            error: error.response?.data?.error?.message || error.message || 'Failed to confirm rewards',
        };
    }
};
