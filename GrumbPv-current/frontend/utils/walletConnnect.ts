...
/**
 * When several wallets inject into the page, `window.ethereum` may not be the MetaMask instance
 * the user actually authorized — session restore and listeners should prefer MetaMask from `providers`.
 */
export const getMetaMaskPreferredEthereum = (): MetaMaskProvider | undefined => {
    ...
    const withProviders = root as MetaMaskProvider & { providers?: MetaMaskProvider[] };
    if (Array.isArray(withProviders.providers)) {
        const mm = withProviders.providers.find((p) => p?.isMetaMask);
        if (mm) {
            return mm;
        }
    }
    if (root.isMetaMask) {
        return root as MetaMaskProvider;
    }
    return undefined;
};
...
export const connectMetaMaskWallet = async (email?: string): Promise<{ address: string; chainId: string } | null> => {
    const provider = getMetaMaskPreferredEthereum() ?? getEthereumProvider();
...
export const connectWallet = async (email?: string): Promise<{ address: string; chainId: string; via: "injected" | "sdk" } | null> => {
    const injected = getMetaMaskPreferredEthereum() ?? getEthereumProvider();
