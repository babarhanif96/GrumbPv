...
export class AdminService {
  ...
  /**
   * Dispute queue — milestones in any disputed state, sorted by urgency.
   */
  public async getDisputesQueue(params: { page?: number; limit?: number; status?: string } = {}) {
    try {
      ...
      const [milestones, total] = await Promise.all([
        this.prisma.job_milestones.findMany({
          where,
          skip,
          take: limit,
        ...
        this.prisma.job_milestones.count({ where }),
      ]);

      const readyToResolve = milestones.filter(
        (m) => m.status === milestone_status.disputedWithCounterSide
      ).length;
      ...
      return {
        data: milestones.map((m) => ({
          ...
          readyToResolve: m.status === milestone_status.disputedWithCounterSide,
        })),
        pagination: {
          page,
          limit,
      ...
      logger.error('Error getting disputes queue', { error });
      throw new AppError('Error getting disputes queue', 500, 'ADMIN_GET_DISPUTES_FAILED');
    }
  }
}
