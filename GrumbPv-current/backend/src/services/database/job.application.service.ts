...
import { isEscrowPaymentTokenSymbol } from '../../config/contracts.js';
import {
  Prisma,
...
export class JobApplicationService {
  ...
  public async updateJobApplication(
    ...
    try {
      ...
      // Do not allow PATCH to reassign parties or move the doc to another job (avoids escrow vs row mismatch).
      delete updateData.client_id;
      delete updateData.job_id;
      delete updateData.freelancer_id;
      ...
      if (updateResult.freelancer_confirm && updateResult.client_confirm) {
        ...
        if (!isEscrowPaymentTokenSymbol(milestone.token_symbol)) {
          throw new AppError(
            'Escrow supports BNB, USDT, or USDC only. Update the payment currency in the reference document.',
            400,
        ...
        const clientBeforeEscrow = await userService.getUserById(jobInfo.client_id);
        const freelancerBeforeEscrow = await userService.getUserById(milestone.freelancer_id);
        if (!clientBeforeEscrow?.address?.trim()) {
          throw new AppError(
            'The job owner (client) must connect a crypto wallet before escrow can be created. Escrow needs the client wallet as the on-chain buyer. Ask the client to connect their wallet once, then confirm again.',
            400,
            'CLIENT_WALLET_REQUIRED_FOR_ESCROW'
          );
        }
        if (!freelancerBeforeEscrow?.address?.trim()) {
          throw new AppError(
            'The freelancer must connect a crypto wallet before escrow can be created. Connect your wallet and try again.',
            400,
            'FREELANCER_WALLET_REQUIRED_FOR_ESCROW'
          );
        }
