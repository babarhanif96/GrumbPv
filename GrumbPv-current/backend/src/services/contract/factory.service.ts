  ...
  async createEscrow(params: CreateEscrowParams): Promise<{
    ...
    try {
      ...
      if (!client?.address?.trim()) {
        throw new AppError(
          'The job owner (client) must connect a crypto wallet before escrow can be created (on-chain buyer). Ask the client to connect their wallet once, then confirm again.',
          400,
      ...
      if (!freelancer?.address?.trim()) {
        throw new AppError(
          'The freelancer must connect a crypto wallet before escrow can be created (on-chain seller).',
          400,
