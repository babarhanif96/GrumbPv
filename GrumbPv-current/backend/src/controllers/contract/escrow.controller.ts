...
import { pinata, uploadPublicFileToPinata } from '../../utils/pinata.js';
...
export class EscrowController {
  ...
  async deliver(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (file) {
        try {
          const uploadResult = await uploadPublicFileToPinata({
            buffer: file.buffer,
  ...
  async getExpectedRewards(req: Request, res: Response, next: NextFunction) {
    try {
      const { job_milestone_id } = req.params;
      const data = await escrowService.getExpectedRewards(job_milestone_id);
      res.json({ success: true, data, message: 'Expected rewards retrieved' });
    } catch (error) {
      next(error);
    }
  }

  async confirmWithdrawRewards(req: Request, res: Response, next: NextFunction) {
    try {
      const { job_milestone_id } = req.params;
      const { txHash } = req.body;
      const data = await escrowService.confirmWithdrawRewards(job_milestone_id, txHash);
      res.json({ success: true, data, message: 'Withdraw rewards confirmed' });
    } catch (error) {
      next(error);
    }
  }

  async verifyRewardInfrastructure(_req: Request, res: Response, next: NextFunction) {
    try {
      const data = await escrowService.verifyRewardInfrastructure();
      res.json({ success: true, data, message: 'Reward infrastructure verified' });
    } catch (error) {
      next(error);
    }
  }
