...
export class AdminController {
  ...
  async getDisputes(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await adminService.getDisputesQueue({
        page: page ? parseInt(page as string) : undefined,
        limit: limit ? parseInt(limit as string) : undefined,
  ...
  async importLegacyUsers(req: Request, res: Response, next: NextFunction) {
    try {
      const { users } = req.body;
      const result = await userService.importLegacyUsers(users);
      res.json({
        success: true,
        data: result,
        message: 'Legacy user import completed',
      });
    } catch (error) {
      next(error);
    }
